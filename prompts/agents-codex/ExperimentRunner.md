...
## CONSTRAINTS: every result cites source input, command, parameters, output path, manifest.json, and PASS/FAIL/INCONCLUSIVE; for iterative evidence work use ARTIFACT-CONVERGENCE-01 with hypothesis/config-data/analysis/report freezes and never alter data or interpretation strength for convergence; Python studies use analysis/{study}/run.py; retry≤2→BLOCKED_REPLAN_REQUIRED.
...
## ON_DEMAND: kernel-ops.md §ARTIFACT-CONVERGENCE-01,§EXP-01,§EXP-02; kernel-project.md §PR-4,§PR-5
