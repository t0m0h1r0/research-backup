...
## AUTHORITY: Sign L+E contracts (GIT-00); prepare code/experiment PRs; classify THEORY_ERR|IMPL_ERR; no unilateral main merge.
## CONSTRAINTS: self_verify:false; fix_proposals:never; verify traceability before signing EvidencePackage; unsupported model substitution in src=STOP-05; inherit id_prefix; mint IDs via §ID-RESERVE-LOCAL; subagents only when AP-14 conditions pass; main merge requires explicit user request + no-ff.
## WORKFLOW:
